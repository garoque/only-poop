...
export default (client: SupabaseClient<Database>) => ({
  ...
  async readAll({ username, from = 0, to = 10 }: ReadAllOptions) {
    const [totalResponse, gistsResponse] = await Promise.all([
      ...
      client
        .from('gists')
        .select('id, title, description, price, lang, is_paid, created_at, profiles!inner(id, username)')
        .eq('profiles.username', username)
  ...
  async update(id: string, { title, description, price, content, lang }: UpdateOptions) {
    ...
    await client.from('gists').update({ title, description, price, content, lang, is_paid: isPaid }).match({ id })
